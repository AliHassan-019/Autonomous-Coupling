...
def load_calib_or_fail(imu, calib_path: Path, label: str):
    ...
    imu.loadCalibDataFromFile(str(calib_path))

def main():
    parser = argparse.ArgumentParser(description="2 MPU9250 + orientation relative (quaternions).")
    ...
    parser.add_argument("--calib1", type=str, default="/home/rem/mpu9250/imu_19/calib_6faces_mag00.json")
    ...
    parser.add_argument("--no_mag_yaw", action="store_true",
    ...
    # Rend l'affichage plus stable (moins d'effet 'paquets')
    try:
    ...
    print(f"Bus=/dev/i2c-{args.bus} addr1={hex(args.addr1)} addr2={hex(args.addr2)}", flush=True)
    ...
    while True:
        ...
        if args.print_mode == "block":
            ...
            # 1 seule ligne => pas d’empilement “bizarre”
            line = (f"IMU1(r={r1:6.2f},p={p1:6.2f},y={y1:7.2f})  "
                    f"IMU2(r={r2:6.2f},p={p2:6.2f},y={y2:7.2f})  "
                    f"REL(r={rr:6.2f},p={pr:6.2f},y={yr:7.2f})")
